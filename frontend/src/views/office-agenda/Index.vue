<template>
  <AdminLayout>
    <div class="page-header">
      ...
      <button @click="openCreateModal" class="btn btn-primary" v-if="canCreate">
        ➕ Tambah Agenda
      </button>
    </div>
    ...
    <Modal v-model="showDetailModal" title="Detail Agenda" width="600px">
      ...
      <div class="form-actions">
        <button @click="deleteAgenda(selectedAgenda)" class="btn btn-delete" v-if="canEdit">
          🗑️ Hapus
        </button>
        <button @click="editAgenda(selectedAgenda)" class="btn btn-primary" v-if="canEdit">
          ✏️ Ubah
        </button>
      </div>
    </Modal>
...
const selectDate = (day) => {
  if (day.agendas.length === 0 && canCreate.value) {
...
const handleSubmit = async () => {
  ...
  try {
    if (editMode.value) {
      await officeAgendaService.update(selectedAgenda.value.id, form.value)
...
const loadAgendas = async () => {
  loading.value = true
  try {
    const response = await officeAgendaService.getAll({ per_page: 1000 })
